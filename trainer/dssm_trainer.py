...
import numpy as np
from models.metric import correct_predictions



class DSSMTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (epitope_encoding, MHC_encoding, target) in enumerate(self.data_loader):
            epitope_encoding= epitope_encoding.to(self.device)
            MHC_encoding = MHC_encoding.to(self.device)
            target = target.to(self.device)
            # print('target',target.shape)
            self.optimizer.zero_grad()
            output = self.model(epitope_encoding, MHC_encoding)
    ...
    def _valid_epoch(self, epoch):
        ...
        correct_output = {'count': 0, 'num': 0}
        with torch.no_grad():
            for batch_idx, (epitope_encoding, MHC_encoding, target) in enumerate(self.valid_data_loader):  
                epitope_encoding= epitope_encoding.to(self.device)
                MHC_encoding = MHC_encoding.to(self.device)
                target = target.to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_idx, (epitope_encoding, MHC_encoding, target) in enumerate(self.test_data_loader):
                epitope_encoding= epitope_encoding.to(self.device)
                MHC_encoding = MHC_encoding.to(self.device)
                target = target.to(self.device)
