...
class EpitopeBertMHCTraniner(BaseTrainer):
    """"
    Trainer class
    """
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (epitope_tokenized, MHC_encoding, target) in enumerate(self.data_loader):
            ...
            output = self.model(epitope_tokenized, MHC_encoding)
    ...
    def _valid_epoch(self, epoch):
        ...
        correct_output = {'count': 0, 'num': 0}
        with torch.no_grad():
            for batch_idx, (epitope_tokenized, MHC_encoding, target) in enumerate(self.valid_data_loader):  
                epitope_tokenized = {k:v.to(self.device) for k,v in epitope_tokenized.items()}
                MHC_encoding = MHC_encoding.to(self.device)
                target = target.to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for _, (epitope_tokenized, MHC_encoding, target) in enumerate(self.test_data_loader):
                epitope_tokenized = {k:v.to(self.device) for k,v in epitope_tokenized.items()}
                MHC_encoding = MHC_encoding.to(self.device)              
                target = target.to(self.device)
