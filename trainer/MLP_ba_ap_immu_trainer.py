...
import models.mlp as module_arch_
...
class EpitopeMHCTraniner(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (epitope_MHC_encoded, target) in enumerate(self.data_loader):
            epitope_MHC_encoded = epitope_MHC_encoded.to(self.device)
            target = target.to(self.device)
            # parent output 
            ba_output, ba_ReLU_output = self.ba_model(epitope_MHC_encoded)
            # print('ba_output shape',ba_ap_output.shape)
            ap_output, ap_ReLU_output = self.ap_model(epitope_MHC_encoded)
            # print('ap_output shape',ba_ap_output.shape)
            self.optimizer.zero_grad()

            output = self.model(ba_ReLU_output, ap_ReLU_output, epitope_MHC_encoded)
    ...
    def _valid_epoch(self, epoch):
        ...
        correct_output = {'count': 0, 'num': 0}
        with torch.no_grad():
            for batch_idx, (epitope_MHC_encoded, target) in enumerate(self.valid_data_loader):  
                epitope_MHC_encoded = epitope_MHC_encoded.to(self.device)
                target = target.to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for _, (epitope_MHC_encoded, target) in enumerate(self.test_data_loader):
                epitope_MHC_encoded = epitope_MHC_encoded.to(self.device)
                target = target.to(self.device)
