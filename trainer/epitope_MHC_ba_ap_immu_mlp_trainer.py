...
class EpitopeMHCTraniner(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (epitope_tokenized, MHC_tokenized, epitope_MHC, target) in enumerate(self.data_loader):
            ...
            epitope_MHC = epitope_MHC.to(self.device)
            # parent output 
            ...
            output = self.model(ba_ReLU_output, ap_ReLU_output, epitope_MHC)
    ...
    def _valid_epoch(self, epoch):
        ...
        correct_output = {'count': 0, 'num': 0}
        with torch.no_grad():
            for batch_idx, (epitope_tokenized, MHC_tokenized, epitope_MHC, target) in enumerate(self.valid_data_loader):  
                epitope_tokenized = {k:v.to(self.device) for k,v in epitope_tokenized.items()}
                MHC_tokenized = {k:v.to(self.device) for k,v in MHC_tokenized.items()}
                target = target.to(self.device)
                epitope_MHC = epitope_MHC.to(self.device)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for _, (epitope_tokenized, MHC_tokenized, epitope_MHC, target) in enumerate(self.test_data_loader):
                epitope_tokenized = {k:v.to(self.device) for k,v in epitope_tokenized.items()}
                MHC_tokenized = {k:v.to(self.device) for k,v in MHC_tokenized.items()}                
                target = target.to(self.device)
                epitope_MHC = epitope_MHC.to(self.device)
