...
import torch
import numpy as np
import data.test_ba_dataset_epitopeBert_MHC as module_data
...
def main(config):
    ...
    test_data_loader = data_loader.get_ba_dataset()
    ...
    test_result_df.to_csv(join(config.save_dir, config['data_loader']['args']['test_file'].replace(".txt",'_EpitopeBert_MHC_result.txt')), index=False)
