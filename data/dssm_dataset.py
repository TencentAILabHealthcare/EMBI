...
class DSSMDataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        epitope_encoding = np.concatenate((epitope_onehot, epitope_BLOSUM50), axis=1)
        epitope_encoding = torch.tensor(epitope_encoding, dtype=torch.float32)
        # print('epitope shape',epitope_encoding.shape)
        # epitope_encoding = torch.tensor(epitope_encoding,dtype=torch.float32)
        # print()
        MHC_encoding = np.concatenate((MHC_onehot, MHC_BLOSUM50), axis=1)
        ...
        # epitope_MHC_encoding = np.concatenate((epitope_encoding, MHC_allele_onehot))
        ...
        return epitope_encoding, MHC_encoding, binder

class DSSMDataLoader(BaseDataLoader):
    ...
    def _encode_24(self, seq):
        left = self._encode_seq(seq[:17])
        right = self._encode_seq(seq[-17:])
        if len(seq) < 17:
            middle = np.zeros((34-len(seq) * 2, 20), dtype='int32')
    ...
    def _encode_24_blosum50(self, seq):
        left = self._encode_seq_blosum50(seq[:17])
        right = self._encode_seq_blosum50(seq[-17:])
        if len(seq) < 17:
            middle = np.zeros((34-len(seq) * 2, 20), dtype='int32')
            merge = np.concatenate((left, middle, right), axis=0)
        else:
    ...
    def _prepare_dataset(self, epitope_BA_df):
        ...
        dataset = DSSMDataset(epitope_onehot, epitope_BLOSUM50, MHC_onehot, MHC_blosum50, binder)
        return dataset