...
class EpitopeMHCBertDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return epitope_tensor, MHC_tensor, binder_tensor, source_tensor                             
...
class EpitopeMHCBertDataLoader(BaseDataLoader):
    def __init__(self, data_dir, batch_size,logger, 
        ...
        super().__init__(self.train_dataset, batch_size, seed, validation_split, test_split, sampler_type, num_workers)
    ...
    def _load_data(self):
        Epitope_BA_df = pd.read_csv(join(self.data_dir, '20221014el_ba_immu_merge_data.csv'),dtype=str)
        test_df = pd.read_csv(join(self.data_dir, '20221011_ba_ap_immu_test_data.csv'),dtype=str)
    ...
    def _process_BA_df(self, Epitope_BA_df):
        ...
        return data_x_epitopes, data_MHC_pseduo_seq, data_Binder, data_source
